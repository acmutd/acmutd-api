...
def get_term_name(term):
    ...
    season = {'s': 'Spring', 'u': 'Summer',
              'f': 'Fall'}.get(season_code, season_code)
    return f"{season} {year}"


def main():
    # Check for environment variables
    if 'GRADE_TERM' not in os.environ:
        print("GRADE_TERM environmental variable not set.")
        exit(1)

    term = os.environ['GRADE_TERM']
    ...
    excel_files = [f for f in os.listdir(
        excel_dir) if f.lower().endswith(('.xlsx', '.xlsb'))]
    if not excel_files:
    ...
    # Output CSV path in out directory
    output_csv = os.path.join("out", f"{term_name}.csv")
