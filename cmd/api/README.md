...
|------------------|---------|----------|--------------------------------------------------|
| `rate_limit`     | integer | yes      | Max requests allowed per window                  |
| `window_seconds` | integer | yes      | Rolling window size in seconds                   |
| `is_admin`       | boolean | no       | Whether the key has admin privileges             |
| `expires_at`     | string  | no       | RFC3339 expiry timestamp; omit for no expiration |
...
- `key` (required): The API key to look up
